...
public class AnswerDao {
    ...
    public void deleteAnswer(String answerUuid) {
        AnswerEntity answerEntity = entityManager.createNamedQuery("getAnswerByUuid", AnswerEntity.class).setParameter("uuid", answerUuid).getSingleResult();
        entityManager.remove(answerEntity);
    }

    public List<AnswerEntity> getAllAnswersToQuestion(String uuid) {
        try {
            return entityManager.createNamedQuery("getAllAnswerstoQuestion", AnswerEntity.class).setParameter("uuid", uuid).getResultList();
